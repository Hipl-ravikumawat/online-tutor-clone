...
class InvoiceService {
  ...
  static async getInvoiceAmountWithinDateRange(studentId, date_range, includeType, categoryIds = [], isCron = false) {
    ...
    const studentBalTillStartDate = await globalHelper.getBalanceDetailsOfStudent(studentId,start_date);
    const studentBalance = await UserService.getStudentBalance([studentId])[0]?.balance || 0;
    ...
    if (includeType === "charges") {
        ...
        const regularTransactions = await Transaction.aggregate([
            {
                $match: {
 ...
 */
static async getRecurringTransactionsInDateRange(studentId, startDate, endDate) {
...
static async generateRecurringInstances(transaction, startDate, endDate) {
  ...
  /**
   * Create invoices for families/students
   */
  static async createInvoices({
    families,
    skip_family_zero_invoice=0,
    invoice_date,
    include_type,
    date_range,
    category,
    ...
    for (const student_id of families) {
        // Void old invoices
        if (voidOld) {
        ...
        await this.getInvoiceAmountWithinDateRange(student_id, date_range, include_type, category, isCron);
        ...
        if(skip_family_zero_invoice == "1" && invoiceAmount == 0){      
            continue;
        }
        ...
        const invoice = new Invoice({
            student_id,
            skip_family_zero_invoice: skip_family_zero_invoice === '1',
        ...
        if (send_email == 1) {
            await mailService.sendInvoiceEmail(invoice, student_id, userDetail);
    ...
    return {
        message:
            families.length > 0
            ? "Invoices created successfully for each student."
...
static async updateRecurringTransactionsWithInvoice(transactions, invoiceId) {
  ...
  static formatDateRange(dateRange) {
    return `${moment.utc(dateRange.start).format("DD-MM-YYYY")} - ${moment.utc(dateRange.end).format("DD-MM-YYYY")}`;
  }

  static async removeInvoiceReferencesFromTransactions(invoiceIds) {
    if (!Array.isArray(invoiceIds) || invoiceIds.length === 0) {
      return;
    }

    await Transaction.updateMany(
      { "invoices_id.id": { $in: invoiceIds } },
      { $pull: { invoices_id: { id: { $in: invoiceIds } } } }
    );
  }

  static async updateEventChargeTransactions(eventIds, studentPricingOption, perStdLessonPrice) {
    ...
    if (studentPricingOption === "std_price_no_charge") {
      await Transaction.updateMany(filter, { $set: { amount: 0 } });
      return;
    }

    if (studentPricingOption === "std_price_specify") {
      const amount = perStdLessonPrice !== undefined && perStdLessonPrice !== null
        ? Number(perStdLessonPrice)
        : null;

      if (amount !== null && !Number.isNaN(amount)) {
        await Transaction.updateMany(filter, { $set: { amount } });
      }
      return;
    }

    if (studentPricingOption === "std_price_default") {
      const events = await Event.find(
        { _id: { $in: eventIds } },
        { duration: 1, is_substitute_tutor: 1, tutor_id: 1, substitute_tutor_id: 1, student_ids: 1 }
      ).lean();
      ...
      events.forEach((event) => {
        const tutorId = event.is_substitute_tutor ? event.substitute_tutor_id : event.tutor_id;
        if (tutorId) tutorIds.add(tutorId.toString());
        (event.student_ids || []).forEach((studentId) => {
          if (studentId) studentIds.add(studentId.toString());
        });
      });

      const assignedTutors = await AssignedTutors.find({
        tutor_id: { $in: Array.from(tutorIds).map((id) => mysqlOrm.Types.ObjectId(id)) },
        student_id: { $in: Array.from(studentIds).map((id) => mysqlOrm.Types.ObjectId(id)) },
      }).lean();

      const assignedMap = new Map();
      assignedTutors.forEach((record) => {
        if (record.tutor_id && record.student_id) {
          assignedMap.set(
            `${record.tutor_id.toString()}_${record.student_id.toString()}`,
      ...
      events.forEach((event) => {
        ...
        const durationHours = (Number(event.duration) || 0) / 60;
        (event.student_ids || []).forEach((studentId) => {
          if (!studentId) return;

          const key = `${tutorId.toString()}_${studentId.toString()}`;
          const hourlyPrice = assignedMap.has(key) ? assignedMap.get(key) : 30;
          const amount = Number((durationHours * hourlyPrice).toFixed(2));
  ...
  static async regenerateInvoicesForEventPriceChange(eventIds, userDetail) {
    if (!Array.isArray(eventIds) || eventIds.length === 0) {
      return { voided: 0, regenerated: 0 };
    }

    const invoices = await Invoice.find({
      "transactions.event_id": { $in: eventIds },
      is_void: { $ne: true },
      isDeleted: false,
    });
    ...
    await this.removeInvoiceReferencesFromTransactions(invoiceIds);

    let regenerated = 0;
    for (const invoice of invoices) {
      await this.createInvoices({
        families: [invoice.student_id],
        skip_family_zero_invoice: invoice.skip_family_zero_invoice ? "1" : 0,
        invoice_date: invoice.date,
        include_type: invoice.include_balance_type,
        date_range: this.formatDateRange(invoice.date_range),
        category: Array.isArray(invoice.category) && invoice.category.length ? invoice.category[0].toString() : null,
        start_date: invoice.date_range.start,
        end_date: invoice.date_range.end,
        due_date: invoice.due_date,
        display_type: invoice.display_type,
        footer_note: invoice.footer_note,
        private_note_text: invoice.private_note,
        send_email: 0,
        send_sms: 0,
        voidOld: false,
        userDetail,
    ...
    return { voided: invoiceIds.length, regenerated };
  }

  /**
   * Check if invoices already exist for given families in a date range
   */
  static async checkExistingInvoices({ families, date_range }) {
    if (!date_range || families.length === 0) {
