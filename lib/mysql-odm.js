...
class Query {
  ...
  async exec() {
    ...
    if (this.options.sort) { const entries = typeof this.options.sort === 'string' ? this.options.sort.split(/\s+/).map(x => [x.replace(/^-/, ''), x.startsWith('-') ? -1 : 1]) : Object.entries(this.options.sort); docs.sort((a,b) => { for (const [k, dir] of entries) { const av=getPath(a,k), bv=getPath(b,k); if(av < bv)return -dir;if(av > bv)return dir; } return 0; }); }
...
async function populateDocs(docs, specs, sourceModel) {
  for (const spec of specs) {
    ...
    for (const doc of docs) {
      ...
      if (spec.populate && populated) await populateDocs(Array.isArray(populated) ? populated : [populated], [spec.populate], target);
    }
  }
}
...
async function aggregate(model, pipeline = []) {
  ...
  for (const stage of pipeline) {
    ...
    else if (op === '$unwind') { const path=(typeof arg==='string'?arg:arg.path).replace(/^\$/,''); const preserve=arg.preserveNullAndEmptyArrays; docs=docs.flatMap(d=>{const a=getPath(d,path);if(!Array.isArray(a)||!a.length)return preserve?[d]:[];return a.map(x=>{const n=clone(d);setPath(n,path,x);return n;});}); }
    else if (op === '$lookup') {
      const target=models.get(arg.from); if(!target)continue; const foreign=(await target._all()).map(x=>x.toObject());
      docs=docs.map(d=>{
        let joined;
        if(arg.pipeline){
          const vars=Object.fromEntries(Object.entries(arg.let||{}).map(([k,v])=>[k,expression(d,v)]));
          joined=foreign.filter(f=>arg.pipeline.every(stage=>!stage.$match||matches(f,stage.$match,vars)));
...
function makeModel(name, schema) {
  ...
  class Model extends Document {
    ...
    static find(f={}, cb) { return withCallback(new Query(Model,'find',f), cb); } static findOne(f={}, cb) { return withCallback(new Query(Model,'findOne',f), cb); }
    static findById(id, cb) { return withCallback(new Query(Model,'findById',{_id:id}), cb); }
