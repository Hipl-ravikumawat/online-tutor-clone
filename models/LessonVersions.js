...
const lessonVersionsSchema = new mysqlOrm.Schema({
    title: {
        type: String,
        required: true,
    },
...
lessonVersionsSchema.pre("save", async function (next) {
    this.slug = uuidv4();
    next();
});


lessonVersionsSchema.pre('findOneAndUpdate', async function (next) {
    const slugId = uuidv4()
...
const Lesson = mysqlOrm.model('lesson_versions', lessonVersionsSchema);
