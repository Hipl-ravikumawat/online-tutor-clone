...
const Practice = require('../../models/TutorTrainingPractice');
...
async function storePractice(req, res) {
    try {
        ...
        let lessonDataFetched = await TutorTrainingLesson.findOne({ "_id": mysqlOrm.Types.ObjectId(lessonId) }).populate('practice_ids');
        let position = 0;
        ...
        const newPractice = await Practice.create(object);
        await TutorTrainingLesson.findOneAndUpdate({ "_id": lessonId }, {
            $push: {
                "practice_ids": newPractice._id
            },
        },{ new: true });

        const crudMessage = "Practice is added successfully!";
    ...
    try {
        ...
        if (req.body.practice_id != '') {
            ...
            if(isAttachedInAssessment){
                crudMessage = "Practice updated successfully! Please remember that this practice is attached to a training assessment!";
                // return res
                // .status(404)
                // .json({ success: false, message: "This practice is enrolled in a training assessment, So it can't be updated."});
            }
            ...
            ImageDir = `./assets/TutorTrainingContent/${practice.content_directory}/`;
...
async function updatePracticePosition(req, res) {
    try {
        ...
        if (positions.length > 0) {
            ...
            const crudMessage = "Practice is reordered successfully!";
            req.flash("success", crudMessage);
            res.status(200).json({ success: true, message: crudMessage });
        } else {
...
async function duplicatePractice(req, res) {
    try {
        ...
        const duplicatePractice = await Practice.create(Obj);
        await TutorTrainingLesson.findByIdAndUpdate(lessonId, { $push: { "practice_ids": duplicatePractice._id } });
        ...
        if (isValidQuestionImage) {
            ...
            fs.copyFile(
                `./assets/TutorTrainingContent/${contentDirectory}/${practice.question_image}`,
                `./assets/TutorTrainingContent/${contentDirectory}/${newQuestionImage}`,
                (err) => {
                  if (err) throw err;
                }
            );
        ...
        if (isValidQuestionAudio) {
            ...
            fs.copyFile(
                `./assets/TutorTrainingContent/${contentDirectory}/${practice.question_audio}`,
                `./assets/TutorTrainingContent/${contentDirectory}/${newQuestionAudio}`,
                (err) => {
                  if (err) throw err;
                }
            );
        ...
        if (practice.options && practice.options.length > 0 && practice.option_display_preference !== 'text') {
            for (option of practice.options) {
                ...
                if (isValidOptionImage) {
                    ...
                    fs.copyFile(
                        `./assets/TutorTrainingContent/${contentDirectory}/${option.option_image}`,
                        `./assets/TutorTrainingContent/${contentDirectory}/${newOptionImage}`,
                        (err) => {
                          if (err) throw err;
                        }
                    );
...
async function destroyPractices(req, res) {
    try {
        ...
        const referencedInAssessment = await Assessment.find({
            "content.lessons": {
                $elemMatch: {
        ...
        if (referencedInAssessment) {
            return res.status(400).json({
                ...
                message:
                  "This practice is currently enrolled in a training assessment and cannot be deleted at this time.",
            });
        } 
        ...
        await TutorTrainingLesson.updateOne({ "slug": lessonSlug }, { $pull: { practice_ids: practiceId } });
...
/**
 * check a practice's answer (non drag & drop).
 * @param {*} req 
 * @param {*} res 
 * @returns 
 */
async function checkPracticeAnswer(req, res) {
    try {
        ...
        const practice = await Practice.findById(req.body.practice_id);
        ...
        if(practice){
            ...
            if (practice.question_type == 'text') {
                ...
                for (practiceData of practice.options) {
                    if (value.includes(practiceData.id)) {
                        data.push({ 'id': practiceData.id, 'description': practiceData.option_explanation });
                    }
                    if (practiceData.option_correct) {
                        correct.push({ 'id': practiceData.id, 'description': practiceData.option_explanation });
                    }
                }
            }
...
/**
 * check drag & drop practice's answer.
 * @param {*} req 
 * @param {*} res 
 * @returns 
 */
...
async function matchTextTypeKeyword(description, keywords){
    ...
    // let textOnly = submittedText.replace(/<p>|<\/p>|<[^>]+>|&nbsp;|<[^>]+&nbsp;/g, ' ');
    // // Convert the text into an array split by spaces
    // let textArray = textOnly.split(/\s+/);  
