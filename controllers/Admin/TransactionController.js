...
async function index(req, res) {
  try {
    return res.render("../views/admin/invoicing/familyAndInvoices/index");
  } catch (error) {
...
async function dataTable(req, res) {
  try {
    ...
    if (user_detail?.role === 2 && typeof attachedStudentIds !== "undefined" && attachedStudentIds?.length) {
    ...
    if (family_or_note) {
      ...
      basePipeline.push({
        $match: {
          $or: [{ note: regex }, { family: regex }, { student_name: regex }],
        },
      });
    }

    // ---- Fetch base results ----
    const baseResults = await Transaction.aggregate([...basePipeline]);
    ...
    const recordsTotal = await Transaction.countDocuments({ isDeleted: false });
...
async function create(req, res) {
  try {
    ...
    // const chargeCategories = await ChargeCategory.find({ deleted_at: null });
    ...
    return res.render("../views/admin/invoicing/familyAndInvoices/transaction/create", {
      paymentMethods,
      chargeCategories,
      taxes,
      families,
      ...
      repeatDays,
      currency,
      familiesObj: JSON.stringify(families) ,
    });
...
async function edit(req, res) {
  try {
    ...
    const editTransaction = await Transaction.findById(id).populate("student_id", "first_name last_name");
    // console.log(editTransaction,"editTransaction");
    
    if (!editTransaction) return res.status(404).json({ message: 'Not found' });
    const studentId = editTransaction?.student_id?._id || null;
    ...
    // const families = await User.find({ company_name: { $exists: true, $ne: "" }, isDeleted: false }, { company_name: 1 });// Get list of families (company users)
    let studentIdArr = [];
    if(studentId){
      studentIdArr = [studentId];
    }
    ...
    if (editTransaction.family) {
      editTransaction.family = editTransaction.family.toString();
    }

    if (Array.isArray(editTransaction.families)) {
      editTransaction.families = editTransaction.families.map(fam => fam.toString());
    }

    return res.render("../views/admin/invoicing/familyAndInvoices/transaction/edit", { 
      paymentMethods, 
      chargeCategories, 
      taxes, 
      families, 
...
async function update(req, res) {
  try {
    ...
    return res.redirect("/families-invoices#Transactions");
  } catch (err) {
